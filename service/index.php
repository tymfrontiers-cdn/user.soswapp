          ...
          <div class="sec-div padding -p20" id="dash-usr">
            <div class="grid-5-tablet grid-12-laptop">
              <div class="align-c color face-primary color-bg" id="avatar-box">
                ...
                <button type="button" onclick="sos.faderBox.url('/app/file-manager/uploader-popup', avatar_param, {exitBtn:true});" id="avatar-set-btn" class="sos-btn face-primary"> <i class="fas fa-edit"></i> Change</button>
              </div>
            </div>
              ...
              <div class="sec-div">
                ...
                <table class="horizontal">
                  <tr title="Account ID/Alias">
                    ...
                    <td><?php echo Data::charSplit($user->id,4,'-'), (!empty($user->alias) ? " (@{$user->alias})" : ""); ?></td>
                  </tr>
                ...
                <button type="button" class="sos-btn sht-btn" name="button" onclick="location.href = '/app/user/settings'"><i class="fas fa-user-cog"></i> Account setting</button>

                <button type="button" class="sos-btn grey sht-btn" onclick="sos.faderBox.url('/app/user/view-profile', {user : '<?php echo $user->id; ?>'}, {exitBtn: true});"> <i class="fas fa-user-circle"></i> View full profile</button>

                <button type="button" onclick="sos.faderBox.url('/app/file-manager/uploader-popup', avatar_param, {exitBtn:true});" id="usr-avatar-set-btn" class="sos-btn sht-btn"> <i class="fas fa-edit"></i> Change profile avatar</button>
              </div>
            </div>
            <br class="c-f">
          </div>
              ...
              <li>
                <a href="<?php echo $dash->path; ?>" <?php if(!empty($dash->onclick)) { echo $dash->onclick; } ?> <?php if ( !empty($dash->classname)) { echo "class='{$dash->classname}'"; } ?>>
                  <span class="fa-stack fa-3x">
                    ...
                    <i class="fas <?php echo $dash->icon; ?> fa-stack-1x fa-inverse"></i>
                  </span>
                </a>
