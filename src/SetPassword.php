...
$user = User::authenticate($session->user->email,$params["password_old"]);
if( !$user ){
  echo \json_encode([
    "status" => "3.1",
...
if (!$database->query("UPDATE {$base_db}.`user` SET password='{$database->escapeValue($pwd)}' WHERE _id = '{$database->escapeValue($user->id)}' LIMIT 1")) {
  echo \json_encode([
    "status" => "4.1",
